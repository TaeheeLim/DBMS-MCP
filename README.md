# 범용 JDBC 읽기 전용 MCP 서버

`Claude  ->  이 MCP 서버  ->  임의의 JDBC DB` 구조의 읽기 전용 데이터베이스 도구.
...
| `run_query(sql, connection?)` | SELECT/WITH 쿼리 실행 (읽기 전용, 행 수 제한) |
...
- **읽기 전용**: SELECT/WITH 외 차단 (INSERT/UPDATE/DELETE/DDL 거부)
- **다중 문장 차단**: `;` 로 이어붙인 쿼리 거부
- **행 수 제한**: `DB_MAX_ROWS` (기본 200)
- **권장**: 전용 **읽기 전용 DB 계정**을 만들어 등록하라. 코드 차단과 DB 권한, 이중으로 막는 것이 안전하다.
